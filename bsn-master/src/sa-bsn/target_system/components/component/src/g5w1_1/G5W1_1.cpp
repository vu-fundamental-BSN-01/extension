#include "component/g3t1_1/G5W1_1.hpp"
...
G5W1_1::G5W1_1(int &argc, char **argv, const std::string &name) :
    Sensor(argc, argv, name, "env_thermometer", true, 1, bsn::resource::Battery("env_thermometer_batt", 100, 100, 1), false),
    markov(),
    dataGenerator(),
    filter(1),
    sensorConfig(),
...
void G5W1_1::setUp() {
    ...
    { // Configure markov chain
        ...
        handle.getParam("envTemp_LowRisk", s);
        lrs = bsn::utils::split(s, ',');
        handle.getParam("envTemp_MidRisk0", s);
        mrs0 = bsn::utils::split(s, ',');
        handle.getParam("envTemp_HighRisk0", s);
        hrs0 = bsn::utils::split(s, ',');
        handle.getParam("envTemp_MidRisk1", s);
        mrs1 = bsn::utils::split(s, ',');
        handle.getParam("envTemp_HighRisk1", s);
...
void G5W1_1::tearDown() {
    Component::tearDown();
}

double G5W1_1::collect() {
    double m_data = 0;
    ros::ServiceClient client = handle.serviceClient<services::WeatherData>("getWeatherData");
    services::WeatherData srv;

    srv.request.vitalSign = "envTemp"; /* env tempt*/
...
double G5W1_1::process(const double &m_data) {
...
void G5W1_1::transfer(const double &m_data) {
    double risk;
    ...
    data_pub = handle.advertise<messages::SensorData>("env_thermometer_data", 10);
...
std::string G5W1_1::label(double &risk) {
    std::string ans;
