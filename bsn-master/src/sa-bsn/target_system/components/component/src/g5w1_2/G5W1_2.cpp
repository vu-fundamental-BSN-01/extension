#include "component/g3t1_1/G5W1_2.hpp"
...
G5W1_2::G5W1_2(int &argc, char **argv, const std::string &name) :
    Sensor(argc, argv, name, "barometer", true, 1, bsn::resource::Battery("altitude_batt", 100, 100, 1), false),
    markov(),
    dataGenerator(),
    filter(1),
    sensorConfig(),
...
void G5W1_2::setUp() {
    ...
    { // Configure markov chain
        ...
        handle.getParam("altitude_LowRisk", s);
        lrs = bsn::utils::split(s, ',');
        handle.getParam("altitude_MidRisk0", s);
        mrs0 = bsn::utils::split(s, ',');
        handle.getParam("altitude_HighRisk0", s);
        hrs0 = bsn::utils::split(s, ',');
        handle.getParam("altitude_MidRisk1", s);
        mrs1 = bsn::utils::split(s, ',');
        handle.getParam("altitude_HighRisk1", s);
...
double G5W1_2::process(const double &m_data) {
...
void G5W1_2::transfer(const double &m_data) {
    double risk;
    ...
    data_pub = handle.advertise<messages::SensorData>("barometer_data", 10);
...
std::string G5W1_2::label(double &risk) {
    std::string ans;
