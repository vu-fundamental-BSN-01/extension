#include "component/g3t1_1/G5W1_3.hpp"
...
G5W1_3::G5W1_3(int &argc, char **argv, const std::string &name) :
    Sensor(argc, argv, name, "humidity", true, 1, bsn::resource::Battery("humidity_batt", 100, 100, 1), false),
    markov(),
    dataGenerator(),
    filter(1),
    sensorConfig(),
...
void G5W1_3::setUp() {
    ...
    { // Configure markov chain
        ...
        handle.getParam("humidity_LowRisk", s);
        lrs = bsn::utils::split(s, ',');
        handle.getParam("humidity_MidRisk0", s);
        mrs0 = bsn::utils::split(s, ',');
        handle.getParam("humidity_HighRisk0", s);
        hrs0 = bsn::utils::split(s, ',');
        handle.getParam("humidity_MidRisk1", s);
        mrs1 = bsn::utils::split(s, ',');
        handle.getParam("humidity_HighRisk1", s);
...
double G5W1_3::process(const double &m_data) {
...
void G5W1_3::transfer(const double &m_data) {
    double risk;
    ...
    data_pub = handle.advertise<messages::SensorData>("hygrometer_data", 10);
